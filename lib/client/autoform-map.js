  ...
  var initTemplateAndGoogleMaps = function() {
    ...
    this.setPlace = (function(_this) {
      return function(place) {
        ...
        _this.$('input[name=formatted_address]').val(place.formatted_address);

        if(!_.isEmpty(place.name)){
          _this.$('input[name=name]').val(place.name);
        }

        if(!_.isEmpty(place.website)){
          _this.$('input[name=website]').val(place.website);
        }

        if(!_.isEmpty(place.international_phone_number)){
          _this.$('input[name=phone_number]').val(place.international_phone_number);
        }else if(!_.isEmpty(place.formatted_phone_number)){
          _this.$('input[name=phone_number]').val(place.formatted_phone_number);
        }
      };
    })(this);

    this.setSearch = (function(_this){
      return function(nameAddress){
        if(_.isArray(nameAddress)){
          var filtered = _.filter(nameAddress, function(e){
            return !_.isEmpty(e);
          });
          _this.$('input.af-map-search').val(filtered.join(', '));
        }else{
          _this.$('input.af-map-search').val(nameAddress);
        }
      }
    }(this));
    ...
    google.maps.event.addListener(searchBox, 'places_changed', (function(_this) {
      return function() {
        ...
        _this.setSearch([place.name, place.formatted_address]);
    ...
    valueOut: function() {
      ...
      if (lat.length > 0 && lng.length > 0) {
        return {
          ...
          website: node.find('input[name=website]').val(),
          phone_number: node.find('input[name=phone_number]').val()
        };
      }
    },
  ...
  Template.afMap.created = function() {
    ...
    return this._interceptValue = function(ctx) {
      ...
      if (t.mapReady.get() && ctx.value && !t._stopInterceptValue) {
        ...
        var name = ctx.value.hasOwnProperty('name') ? ctx.value.name : null;
        var formatted_address = ctx.value.hasOwnProperty('formatted_address') ? ctx.value.formatted_address : null;
        t.setSearch([name, formatted_address]);
